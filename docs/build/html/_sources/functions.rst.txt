...
#. Удаление пунктуации и приведение к нижнему регистру: 
    ...
    Из входной строки **posname** удаляются символы, обозначающие пунктуацию и не используемые в названиях стран. Для реализации используется список символов 
...
#. Поиск совпадения всей строки:
    ...
    2.1. И ключа с приоритетом "1":
		 ...
		 между строками ключ с приоритетом **1** из нашей базы данных. При этом проверяем разность длин совпавшего ключа и входной строки. Если эта разность по модулю 
		 не больше 1, то есть длины примерно равны, то значение по полученному ключу возвращается функцией и ее работа завершается.
